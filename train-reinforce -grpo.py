...
def train_reinforce():
    ...
    TOTAL_EPISODES = 100000 # Total num of envr rollout episodes to train
    MAX_STEPS = 70
    LOG_INTERVAL = 100 # Log interval to W&B -> every 10 episodes
    EVAL_INTERVAL = 1000 # Evaluate on held-out test mazes every 100 episodes
    ...
    run_name = f"RL_{ALGORITHM}_G{GROUP_SIZE}_{D}x{D}_100k" if ALGORITHM != "REINFORCE" else f"RL_{'REINFORCE_Baseline_CC:' + str(CRITIC_COEFF) if USE_BASELINE else 'Vanilla_REINFORCE'}_{D}x{D}"
